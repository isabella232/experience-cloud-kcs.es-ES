...
En la carga útil anterior, la variable `urn:maxCount00008/urn:maxCount` toma un valor numérico. Si utilizamos un 0 final, funciona bien hasta 0007, p. ej. (0000,0001,0002...0007)
...
Al comprobar la biblioteca Interaction.js, vemos que la variable `parseInt()` se utiliza para analizar la variable `maxCount` como entero. Esta `parseInt()` devuelve un valor decimal de forma predeterminada.
...
si `maxCount`, pasamos 8 en lugar de 0008, obtenemos la propuesta en la respuesta a la llamada SOAP.
